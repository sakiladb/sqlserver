...
> [!TIP]
> Building or testing on GitHub Actions? Pull from GHCR (`ghcr.io/sakiladb/sqlserver`). Docker Hub
...
- **Aligned `sales_by_store` with the canonical Sakila** (`v2022.0.2`, `v2019.0.5`): dropped a stray
  leading `store_id` column from the view, so it is now `store, manager, total_sales`, matching the
  column set across the rest of the family.
