...
cat /sakila/1-sql-server-sakila-schema.sql \
  /sakila/2-sql-server-sakila-insert-data.sql \
  /sakila/3-sql-server-sakila-user.sql >/sakila/init-db-full.sql
...
for i in {1..50}; do
  sqlcmd -S localhost -U sa -i /sakila/init-db-full.sql
  if [ $? -eq 0 ]; then
    break
  else
    echo "Waiting..."
